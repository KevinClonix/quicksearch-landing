...
현재 배포 파일은 `QuickSearch-Setup-0.1.0-20260921-r3-x64.exe`입니다. 재부팅 후 검색 준비 안내, Store 앱 설치 위치 조회 개선, 앱 정보의 지속 캐시, 일반 프로그램 목록 우선 표시를 포함합니다. 내부·시스템 구성요소의 상세 정보는 포함 옵션을 켰을 때 수집합니다. 제거 시 업데이트 백업과 검색 색인·앱 정보 캐시를 정리하고 API 키·파일 형식 설정의 삭제 여부를 선택할 수 있습니다. 원본 파일과 다른 사용자의 데이터는 보존합니다.

설치 EXE의 SHA-256은 `850fead2fcd75db43b7f6f0208681c8f1dfcec77d87465aaefb19e4687654f10`입니다. 개발 PC에서 서비스 재시작 후 디스크 캐시를 재사용한 목록 조회는 0.390초였습니다. 이는 재부팅 직후 또는 모든 PC의 성능 보장 수치는 아닙니다.
...
python scripts/prepare-landing-release.py --release 20260921-r3
```

준비 스크립트는 설치 파일의 SHA-256을 검증하고, 다운로드 파일·체크섬·페이지 링크·표시 용량을 갱신합니다. 이전 다운로드 파일은 정리하며, 로컬 링크를 검사한 뒤 `dist/QuickSearch-LandingPage-20260921-r3.zip`과 ZIP 체크섬을 만듭니다. ZIP에는 정적 사이트와 배포 파일만 포함됩니다. 압축을 풀어 사이트 루트로 사용할 수 있으며, Git 이력·개발 소스·개인 설정은 포함하지 않습니다. 이 단계는 외부 업로드나 실제 배포를 수행하지 않습니다.
